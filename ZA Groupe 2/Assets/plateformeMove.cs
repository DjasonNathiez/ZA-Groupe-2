...
public class plateformeMove : MonoBehaviour
{
    ...
    {
        Gizmos.DrawWireCube(new Vector3(transform.position.x,transform.position.y + yTest/2,transform.position.z),new Vector3(rect.width,yTest,rect.height) );
    }

    public void Update()
    {
        rect.position = new Vector2(transform.position.x - transform.localScale.x / 2, transform.position.z - transform.localScale.z / 2);
