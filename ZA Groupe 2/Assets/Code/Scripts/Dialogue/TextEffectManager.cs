...
public class TextEffectManager : MonoBehaviour
{
    ...
    public float speedOfShowing = 0.005f;
            ...
            for (int i = 0; i < textInfo.characterCount; i++)
            {
                ...
                if (dialogue.Length > 0)
                {
                    for (int v = 0; v < 4; v++)
                    {
                        ...
                        newcolors[charInfo.vertexIndex + v] = Color32.Lerp(
                            new Color32(original.r, original.g, original.b, 0), original, charBasedHeight[i] + 1);
                    }
                }
            }
    ...
    public void UpdateColor()
    {
        ...
        for (int i = 0; i < textInfo.characterCount; i++)
        {
            ...
            if (dialogue.Length > 0)
            {
                ...
                {
                    if (i >= dialogue[dialogueIndex].textEffects[e].firstCharIndex &&
                        i <= dialogue[dialogueIndex].textEffects[e].lastCharIndex)
                    {
                        for (int v = 0; v < 4; v++)
                        {
                ...
                {
                    Color32 original = colors[charInfo.vertexIndex + v];
                    newcolors[charInfo.vertexIndex + v] =
                        Color32.Lerp(new Color32(original.r, original.g, original.b, 0), original,
                            charBasedHeight[i] + 1);
                }
            }
        }
    ...
    public Vector3 ApplyEffectToVertex(Vector3 original, EffectTypeEnum effect, Vector3 center, float speed,
        float width, float height)
    {
        ...
        switch (effect)
        {
            ...
            case EffectTypeEnum.ZOOM:
                product = (center + (product - center) * Mathf.Sin(Time.time * speed + center.x * width) * 0.2f) +
                          (product - center) * 1.1f;
                break;
    ...
    public Color32 ApplyColorEffectToVertex(ColorEffectTypeEnum effect, Color32 first, Color32 second, float speed,
        float width, Vector3 original)
    {
...
public class DialogueLine
{
    ...
    [FormerlySerializedAs("m_textEffects")]
