...
public class TestRope : MonoBehaviour
{
    ...
    private void DORopeExtension()
    {
        ...
        if (nodes.Count > 0)
        {
            ...
            {
                float angleDiffNode = Vector3.SignedAngle((rope.GetPosition(i - 1) - rope.GetPosition(i - 2)).normalized,
        ...
        if (pinnedToObject)
        {
            ...
            {
                Vector3 force = (rope.GetPosition(1) - rope.GetPosition(0)).normalized * 0.01f *
                                (lenght - pinnedObjectDistance);
                pinnedRb.AddForceAtPosition(force, pin.transform.position, ForceMode.VelocityChange);
            }
        }

        if (Vector3.Distance(rope.GetPosition(rope.positionCount - 2), rope.GetPosition(rope.positionCount - 1)) >
            remainingLenght)
        {
            if (pinnedToObject)
            {
                pinnedRb.AddForceAtPosition(
                    (rope.GetPosition(1) - rope.GetPosition(0)).normalized * 0.01f *
                    GetComponent<PlayerManager>().m_rb.velocity.magnitude, pin.transform.position,
